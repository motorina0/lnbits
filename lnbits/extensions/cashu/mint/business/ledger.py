...
# from lnbits.settings import WALLET # revisit
...
class Ledger:
    def __init__(self, secret_key: str, db: str, wallet_id: str):
    ...
    async def _request_lightning_invoice(self, amount: int):
        ...
        _, payment_request, checking_id = await create_invoice(
            wallet_id=self.wallet_id,
            amount=amount,
            memo= "cashu deposit",
            extra={"tag": "cashu", "cashuId": "cashu_id_temp"},
        )
    ...
    async def _check_lightning_invoice(self, payment_hash: str):
        ...
        status = await check_transaction_status(self.wallet_id, payment_hash)
        if status.paid:
    ...
    async def _pay_lightning_invoice(self, invoice: str, amount: int):
        ...
        # if error:
        #     raise Exception(f"Lightning wallet not responding: {error}")
        # ok, _, _, preimage, _ = await WALLET.pay_invoice(
        #     invoice, fee_limit_msat=fee_reserve(amount * 1000)
        # )
        _, ok, preimage = await pay_invoice(
            wallet_id=self.wallet_id,
            payment_request=invoice,
            fee_limit_msat=fee_reserve(amount * 1000),
            extra={"tag": "cashu"},
        )
        return ok, preimage
